...
public class WeaponItem extends Item {
    ...
    @Override public InteractionResultHolder<ItemStack> use(Level level,net.minecraft.world.entity.player.Player player,InteractionHand hand){ItemStack stack=player.getItemInHand(hand);if(level.isClientSide)return InteractionResultHolder.success(stack);if(!(player instanceof ServerPlayer sp))return InteractionResultHolder.fail(stack);var m=FortsMatchManager.get(sp.server);var team=m.teamOf(sp.getUUID());var bank=m.bank(team);if(bank==null){sp.sendSystemMessage(Component.literal("§cСначала войдите в команду"));return InteractionResultHolder.fail(stack);}if(technology!=null&&!bank.has(technology)){sp.sendSystemMessage(Component.literal("§cНе изучена технология: "+technology));return InteractionResultHolder.fail(stack);}if(!bank.spend(0,energy)||!bank.spendAmmo(ammo)){sp.sendSystemMessage(Component.literal("§cНедостаточно энергии или боеприпасов"));return InteractionResultHolder.fail(stack);}Vec3 look=sp.getLookAngle();LargeFireball shot=new LargeFireball(level,sp,look.normalize(),explosion);shot.setPos(sp.getX()+look.x*1.5,sp.getEyeY()-0.1+look.y*1.5,sp.getZ()+look.z*1.5);shot.setDeltaMovement(look.scale(velocity));level.addFreshEntity(shot);sp.getCooldowns().addCooldown(this,Math.max(4,explosion*3));return InteractionResultHolder.consume(stack);}
}
